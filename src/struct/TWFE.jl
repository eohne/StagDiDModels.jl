...
"""
    TWFE <: RegressionModel
"""
struct TWFEModel <: RegressionModel
...
StatsAPI.r2(m::TWFEModel) = m.r2
StatsAPI.islinear(::TWFEModel) = true
function StatsAPI.adjr2(m::TWFEModel)
    return m.adjr2
end


# F-statistic
function fstatistic(m::TWFEModel)
    r² = StatsAPI.r2(m)
    p = StatsAPI.dof(m) 
    n = StatsAPI.nobs(m)
    df_reg = p
...
function StatsAPI.coeftable(m::TWFEModel)
    β = StatsAPI.coef(m)
    se = StatsAPI.stderror(m) 
    ts = tstatistic(m)
...
function top(m::TWFEModel)
    out = [
            "Number of obs" sprint(show, nobs(m), context = :compact => true);
            "R²" @sprintf("%.3f",r2(m));
            "Post obs" sprint(show, m.n_post, context = :compact => true);
...
function Base.show(io::IO, m::TWFEModel)
    ct = coeftable(m)
...
"""
    TWFEModel(β, Σ, names, nobs, dof_resid; kwargs...)

Construct a TWFEModel with required statistics and optional diagnostics.
"""
function TWFEModel(β::AbstractVector, Σ::AbstractMatrix, names::AbstractVector,
